...
From my day-to-day life, content moderation and recommendation engines in social media sites exemplify how databases and data systems quietly encode values about what society should prioritize. Recommendation engines, like Facebook/Instagram feeds, are built on enormous databases that log billions of micro-interactions every second, which include every scroll, pause, like, comment, share, watch-time second, and reaction. These logs are stored, processed, and fed into machine-learning models that predict contents that maximize user engagement. Content moderation, which underlies rules and systems that remove, demote, or label posts, runs in parallel to decide what to suppress. The underlying data systems are optimized for speed and scale. Platforms’ business model is advertising revenue, which depends on time on site and user retention. The algorithms therefore treat engagement as the primary measure for success. This embeds a value system where profit via attention is prioritized over truth, civic health, well-being, or democratic discourse. Hence, content that triggers outrage or polarization reliably outperforms calm and factual content. Because stronger moderation for civic health often reduces user time spentm which hurts revenue, social media companies frequently choose to promote more rage-bait to keep users scrolling. I can imagine that marginalized communities often face disproportionate harm from this system as echo chambers and radicalization pathways tend to pull in vulnerable users faster than they affect the average user. Therefore, the algorithmic amplification of outrage can exacerbate existing social inequalities by making already-polarized topics even more salient, while quieter, community-building voices from underrepresented groups get buried. In short, the databases powering these systems encode a worldview that treats users primarily as attention-generating assets rather than citizens or human beings. Thus, they rigidify emotional and ideological structures because those are the most “engaging”.
